...
import { useIsMounted, useIsomorphicLayoutEffect, useWindowSize } from "usehooks-ts";
...
const ToolbarComponent: React.FC = () => {
  ...
  return (
    <div data-inspectable="false" className="flex flex-col items-center justify-evenly relative z-50">
      <div
        className={cn(
      ...
      >
        <Button
          className={cn("h-10 w-10 transition-width ease-in-out duration-300", isToolbarOpen && "rounded-r-none w-8")}
          onClick={() => setToolbarOpen(!isToolbarOpen)}
          size="icon"
        >
          {isComponentMounted() && activePlugins.responsiveIndicator ? (
            <ResponsiveIndicator className={cn("h-8 transition-width duration-300", isToolbarOpen ? "w-6" : "w-10")} />
          ) : (
            <Icons.arrow
              className={cn("transition-all ease-in-out duration-300 w-10", isToolbarOpen && "-rotate-180 w-6")}
            />
          )}
        </Button>
        {isToolbarOpen && (
          <div className="h-10 flex">
            {activePlugins.inspect && <ClassPanelActivator isOpen={isToolbarOpen} />}
            <PluginsPopup isPopoverOpen={isPopoverOpen} setPopoverOpen={setPopoverOpen} isToolbarOpen={isToolbarOpen} />
            {selectedElement ? (
...
export const Toolbar: React.FC = () => {
  const [position, setPosition] = useState({ top: "1.25rem", left: "1.25rem" });
  ...
  const onMouseMove = (e: MouseEvent) => {
    let newPosition = {
      left: Math.max(
        SAFE_AREA,
        Math.min(e.clientX - dragOffset.current.x, width - toolbarRef.current!.offsetWidth - SAFE_AREA)
      ),
      top: Math.max(
        SAFE_AREA,
        Math.min(e.clientY - dragOffset.current.y, height - toolbarRef.current!.offsetHeight - SAFE_AREA)
      ),
    };
    ...
    magneticPositions.forEach((pos) => {
      ...
      if (Math.abs(newPosition.left - magneticLeft) < SNAP_THRESHOLD) {
        newPosition.left = Math.max(
          SAFE_AREA,
          Math.min(magneticLeft, width - toolbarRef.current!.offsetWidth - SAFE_AREA)
        );
      }
      if (Math.abs(newPosition.top - magneticTop) < SNAP_THRESHOLD) {
        newPosition.top = Math.max(
          SAFE_AREA,
          Math.min(magneticTop, height - toolbarRef.current!.offsetHeight - SAFE_AREA)
        );
      }
    });
...
export const ClassPanelActivator: React.FC<InspectorButtonProps> = ({ isOpen }) => {
  ...
  return (
    ...
    >
      ...
      <span className="relative ml-2 flex h-2 w-2">
        ...
        <span
          className={cn("relative inline-flex rounded-full h-2 w-2", isInspectorActive ? "bg-green-500" : "bg-red-500")}
        />
      </span>
    </Button>
  );
